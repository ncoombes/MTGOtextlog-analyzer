...
color_dict = ['W/B', 'B/R', 'W/U', 'G/U','R/G','B','G','U','W','R']
...
#function for turning the mtgsdk mana cost column into generic converted mana cost( formated {num}{color}{color} etc. can handle hybrid mana of form {W/B} as long as color_dict variable is properly defined earlier to include all 10 possible formats. This ignores variable costs of {x} for spells like fireball for now.
def cmc_col_gen (mana_cost):
    mana_as_num=replaceMultiple(str(mana_cost), color_dict, '1').replace('{','').replace('X','').replace('None','').split('}')
